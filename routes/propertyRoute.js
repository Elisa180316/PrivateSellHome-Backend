...
propertyRoute.get("/find/latest", async (req, res) => {
  try {
    const latestProperties = await Property.find({ latest: true }).populate(
      "seller"
    );
...
propertyRoute.get("/find/byquery", async (req, res) => {
  ...
  if (priceRange) {
    ...
    query.price = { $gte: minPrice?.replace(/[.,]/g, ''), $lte: maxPrice?.replace(/[.,]/g, '') };
  }
...
propertyRoute.patch("/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (property.seller.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ error: "You don't have permission to make changes" });
    }
